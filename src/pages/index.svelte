...
<script>
  ...
  onMount(async () => {
    const { data, error } = await supabase
      .from("Famille")
      .select("id,membre,image,position");
    console.log(data);
    Famille = data;
  });
</script>
	...
	<section class="grid-wrap">
      ...
      <div class= "container">
          ...
          <input type="text" placeholder="Commentaire" name=commentaire required>					
					...
					<button on:submit onclick="closeForm()">Me contacter</button>
...
<script>
  ...
  function closeForm() {
	...
	{#each Famille as { membre, image }}
		<div class="texte texte1 grid-col col-3 med-4 sml-6 xs-12">
			<h2>{membre}</h2>
			<div class="Famille">
